...
import { getGuardrailsByPlatform, getUploadedGuardrails, type GuardrailPlatform } from '@/lib/guardrails'
...
export async function GuardrailList({ platform, label }: { platform: GuardrailPlatform; label: string }) {
  const grouped = getGuardrailsByPlatform(platform)
  const categories = Object.keys(grouped)
  const uploaded = await getUploadedGuardrails(platform)

  if (categories.length === 0 && uploaded.length === 0) {
    return (
      <EmptyState
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              <div key={doc.slug}>
                {doc.severity ? (
                  <div className="checklist-row">
                    <span className="checklist-row-glyph" />
                    <span className="checklist-row-text">{doc.category || 'Uploaded spec'}</span>
                    <Badge variant={SEVERITY_VARIANT[doc.severity] ?? 'neutral'}>{doc.severity}</Badge>
                  </div>
                ) : null}
                {doc.body ? (
                  <div className="guardrail-rule-body">
